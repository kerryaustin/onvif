 ...
 */
 Cam.prototype._passwordDigest = function() {
  ...
  if (!timestamp) {
      ...
      cryptoDigest.update(nonce + unescape(encodeURIComponent(timestamp)) + unescape(encodeURIComponent(this.password)));
 ...
 */
 Cam.prototype._envelopeHeader = function() {
  ...
  var head = "<SOAP-ENV:Envelope xmlns:SOAP-ENV=\"http://www.w3.org/2003/05/soap-envelope\"";
  head += "xmlns:wsse=\"http://docs.oasis-open.org/wss/2004/01/oasis-200401";
  head += "-wss-wssecurity-secext-1.0.xsd\"";
  head += "xmlns:wsu=\"http://docs.oasis-open.org/wss/2004/01/oasis-";
  head += "200401-wss-wssecurity-utility-1.0.xsd\"";
  head += "xmlns:tds=\"http://www.onvif.org/ver20/ptz/wsdl\">"; 
  head += "<SOAP-ENV:Header><wsse:Security><wsse:UsernameToken>"; 
  head += "<wsse:Username>" + this.username + "</wsse:Username>"; 
  head += "<wsse:Password Type=\"http://docs.oasis-open.org/wss/2004/01/oasis-";
  head += "200401-wss-username-token-profile-1.0#PasswordDigest\">";
  head += req.passdigest + "</wsse:Password><wsse:Nonce>" + req.nonce + "</wsse:Nonce>"; 
  head += "<wsu:Created>" + req.timestamp + "</wsu:Created></wsse:UsernameToken>";
  head += "</wsse:Security></SOAP-ENV:Header><SOAP-ENV:Body>";
  return head;
};
